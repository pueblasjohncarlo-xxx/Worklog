<div x-cloak class="app-sidebar fixed inset-y-0 left-0 h-full w-64 bg-black text-gray-100 flex flex-col z-40 shadow-xl overflow-y-auto glow-border-right transform transition-transform duration-300 md:translate-x-0" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
    ...
    <nav class="flex-1 mt-6 px-4 space-y-1" @click="if (window.innerWidth < 768) sidebarOpen = false">
        ...
        <a href="{{ route('ojt_adviser.dashboard') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.dashboard') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
        ...
        <a href="{{ route('ojt_adviser.students') }}" 
           class="flex items-center justify-between px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.students*') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
            ...
            @php
                $studentCount = \App\Models\Assignment::query()
                    ...
                    ->count('student_id');
            @endphp
            <span class="bg-gray-800 text-gray-100 text-xs px-2 py-0.5 rounded-full">{{ $studentCount }}</span>
        </a>

        <a href="{{ route('ojt_adviser.accomplishment-reports') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.accomplishment-reports') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <a href="{{ route('ojt_adviser.mapping.index') }}" 
              class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.mapping.*') || request()->routeIs('ojt_adviser.mopping.*') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
        ...
        <a href="{{ route('ojt_adviser.evaluations') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.evaluations*') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
        ...
        <a href="{{ route('ojt_adviser.reports') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('ojt_adviser.reports*') ? 'bg-indigo-900 text-white shadow-lg' : 'hover:bg-gray-900 text-gray-200' }}">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-4 border-t border-indigo-500/30">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors hover:bg-red-900/30 text-red-300">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
