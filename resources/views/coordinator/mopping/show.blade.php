                                        ...
                                        <tr>
                                            ...
                                            <td class="space-x-2 px-4 py-3 text-right text-sm">
                                                @if($log->attachment_path)
                                                    <a href="{{ route('coordinator.worklogs.attachment', ['workLog' => $log->id, 'inline' => 1, 'v' => optional($log->updated_at)->timestamp ?? $log->id]) }}" target="_blank" class="inline-flex items-center rounded-md bg-slate-900 px-3 py-1.5 text-xs font-bold !text-white shadow-sm ring-1 ring-slate-700 transition-colors hover:bg-slate-800 hover:!text-white focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 active:bg-slate-950 active:!text-white disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-100 dark:!text-slate-950 dark:ring-slate-300 dark:hover:bg-white dark:hover:!text-slate-950 dark:focus:ring-slate-500 dark:focus:ring-offset-gray-800 dark:active:bg-slate-200 dark:active:!text-slate-950">View</a>
                                                @else
