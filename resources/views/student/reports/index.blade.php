    ...
    <div class="space-y-6">
        ...
        @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div x-data="{ percentage: {{ (float)$progressPercentage }} }" class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    ...
                    <p class="text-[10px] text-gray-500 mt-3 font-bold uppercase tracking-wider">Goal: {{ $assignment?->required_hours ?? 1600 }} hrs</p>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    ...
                    <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wider">For {{ now()->format('F Y') }}</p>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    ...
                    <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wider">To complete OJT</p>
                </div>
            </div>

            <!-- Filter Section -->
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-6">Report Options</h2>
                <form action="{{ route('student.reports.index') }}" method="GET" class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="space-y-2">
                            <label for="start_date" class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">Start Date</label>
                            <input type="date" id="start_date" name="start_date" value="{{ request('start_date') }}" class="block w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                        </div>
                        <div class="space-y-2">
                            <label for="end_date" class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">End Date</label>
                            <input type="date" id="end_date" name="end_date" value="{{ request('end_date') }}" class="block w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                        </div>
                        <div class="space-y-2">
                            <label for="status" class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">Status</label>
                            <select id="status" name="status" class="block w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                    ...
                    @if(request()->anyFilled(['start_date', 'end_date', 'status']))
                        <div class="pt-2">
                            <a href="{{ route('student.reports.index') }}" class="inline-block px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                ✕ Clear Filters
                            </a>
                        </div>
                    @endif
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm divide-y divide-gray-100 dark:divide-gray-800">
                                <thead class="bg-gray-50 dark:bg-gray-900/50">
                                    <tr>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Date</th>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Time In</th>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Time Out</th>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Hours</th>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Status</th>
                                        <th class="px-6 py-4 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-xs">Description</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    @foreach($workLogs as $log)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            <td class="px-6 py-4 font-semibold text-gray-800 dark:text-gray-200">{{ $log->work_date->format('M d, Y') }}</td>
                                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $log->time_in ? \Carbon\Carbon::parse($log->time_in)->format('h:i A') : '-' }}</td>
                                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $log->time_out ? \Carbon\Carbon::parse($log->time_out)->format('h:i A') : '-' }}</td>
                                            <td class="px-6 py-4 font-mono font-bold text-indigo-600 dark:text-indigo-400">{{ number_format($log->hours, 2) }}</td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4 text-gray-600 dark:text-gray-400 max-w-xs truncate" title="{{ $log->description }}">
                                                {{ $log->description ?? '-' }}
                                            </td>
                                        </tr>
                                    @endforeach
